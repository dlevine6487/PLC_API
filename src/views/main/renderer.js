...
function main() {
    ...
    function showToast(options) {
        const { title, details, type = 'success', duration = 3000 } = options;
        ...
        if (duration > 0) {
            content += `<div class="toast-timer" id="${timerId}">(Closing in ${duration / 1000}s...)</div>`;
        }
    ...
    function showLoading() { D.loadingOverlay.classList.add('active'); }
    function hideLoading() { D.loadingOverlay.classList.remove('active'); }
    ...
    D.pollRateSelect.addEventListener('change', (e) => {
        ...
        D.pollingRateDisplayEl.textContent = `${e.target.value / 1000}s`;
    });
    ...
    D.dropZone.addEventListener('dragover', (e) => {
        e.preventDefault(); e.stopPropagation(); D.dropZone.classList.add('dragover');
    });
    D.dropZone.addEventListener('dragleave', (e) => {
        e.preventDefault(); e.stopPropagation(); D.dropZone.classList.remove('dragover');
    });

    // --- Initial UI State ---
    D.appContainer.style.display = 'none';

    // --- Main Backend State Listener ---
